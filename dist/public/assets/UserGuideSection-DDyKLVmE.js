import{aV as X,B as o,u as V,j as e,at as P,au as T,bR as ee,aG as G,aH as F,bO as ne,X as E,ag as W,W as w,cr as te,$ as re,ci as ae,ac as ie,c9 as oe,aD as se}from"./index-DWpmCORb.js";import{P as le}from"./progress-BdeebGxe.js";import{A as ce,a as de,b as ue,c as he}from"./accordion-BjiS_sLe.js";import{A as pe}from"./award-DGfs9y48.js";import{M as B,r as U}from"./index-BdqYoOVR.js";/**
 * @license lucide-react v0.453.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const L=X("Trophy",[["path",{d:"M6 9H4.5a2.5 2.5 0 0 1 0-5H6",key:"17hqa7"}],["path",{d:"M18 9h1.5a2.5 2.5 0 0 0 0-5H18",key:"lmptdp"}],["path",{d:"M4 22h16",key:"57wxv0"}],["path",{d:"M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22",key:"1nw9bq"}],["path",{d:"M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22",key:"1np0yb"}],["path",{d:"M18 2H6v7a6 6 0 0 0 12 0V2Z",key:"u46fv3"}]]),me=`# VaxPlan — End-User Guide\r
\r
...
`,ge=`# VaxPlan — Facility Staff Quick-Start\r
\r
...
`,Y=[{id:"quickstart",name:"Quick-Start Pro",description:"Read the Facility Quick-Start guide.",icon:"⚡",color:"from-amber-400 to-orange-500"},{id:"gis_intel",name:"GIS Navigator",description:"Complete the Settlement Intelligence section and pass the quiz.",icon:"🛰️",color:"from-sky-400 to-indigo-500"},{id:"routine_plan",name:"Field Commander",description:"Complete the Routine Microplanning section and pass the quiz.",icon:"🗺️",color:"from-emerald-400 to-teal-500"},{id:"scholar",name:"Wiki Scholar",description:"Mark all available wiki user guide sections as read.",icon:"🎓",color:"from-violet-400 to-purple-500"}],ye={"11-settlement-intelligence-and-zero-dose-targeting":{id:"gis_intel",title:"Settlement Intelligence & Zero-Dose Quiz",questions:[{question:"What does the Outreach Site Suitability Score represent?",options:["The percentage of completed supervision visits.","An abstract population density indicator.","A 0-100 score prioritizing unserved building clusters based on size, zero-dose risk, distance, and road travel time."],correctAnswer:2,explanation:"The Outreach Site Suitability Score aggregates multiple factors (unserved size, zero-dose children, distance, accessibility) to help planners choose the optimal location for new outreach sessions."},{question:"What is the spatial resolution of the WorldPop gridded population data in VaxPlan?",options:["100 meters × 100 meters (approx. 1 hectare)","1 kilometer × 1 kilometer","5 kilometers × 5 kilometers"],correctAnswer:0,explanation:"VaxPlan fuses high-resolution WorldPop raster data, which maps population density at 100m grid cells, letting planners click the map and get a precise headcount of people."}]},"5-facility-staff--your-daily-workflow":{id:"routine_plan",title:"Routine Microplanning Quiz",questions:[{question:"How far in advance must a vaccination session date be scheduled?",options:["At least 24 hours in advance","At least 7 days in advance","No advance scheduling is required"],correctAnswer:1,explanation:"To allow for logistics and cold chain planning, all itinerary days must be scheduled at least 7 days in the future."},{question:"Which role is responsible for reviewing and approving microplans?",options:["Facility Clerks","WHO external monitors only","District Managers and Provincial Coordinators"],correctAnswer:2,explanation:"Authoring is done at the facility level, while review and approvals are routed hierarchically to District Managers and Provincial Coordinators."}]}};function fe(u){return u.toLowerCase().replace(/[^a-z0-9\s-]/g,"").trim().replace(/\s+/g,"-")}function ve(u){const y=u.split(`
`),h=[];let d=null;for(const i of y){const f=i.match(/^##\s+(.+?)\s*$/);if(f){d&&h.push(d),d={id:fe(f[1]),title:f[1],level:2,body:""};continue}d&&(d.body+=i+`
`)}return d&&h.push(d),h.filter(i=>!/table of contents/i.test(i.title))}function Ce({isFacilityRole:u}){const y=o.useMemo(()=>ve(me),[]),[h,d]=o.useState(""),[i,f]=o.useState(void 0),[a,N]=o.useState([]),[m,j]=o.useState([]),[I,H]=o.useState({}),[Q,M]=o.useState({}),[q,b]=o.useState(null);o.useEffect(()=>{try{const n=localStorage.getItem("vaxplan.docs.readSections");n&&N(JSON.parse(n));const t=localStorage.getItem("vaxplan.quizzes.completed");t&&j(JSON.parse(t))}catch{}},[]);const x=n=>{N(n);try{localStorage.setItem("vaxplan.docs.readSections",JSON.stringify(n))}catch{}},{data:v=[],isLoading:$}=V({queryKey:["/api/wiki/pages"],queryFn:async()=>{const n=await fetch("/api/wiki/pages");if(!n.ok)throw new Error("Failed to fetch wiki list");return(await n.json()).data},retry:1}),{data:g}=V({queryKey:["/api/wiki/pages",i],queryFn:async()=>{if(!i||i==="quickstart")return null;const n=await fetch(`/api/wiki/pages/${encodeURIComponent(i)}`);if(!n.ok)throw new Error("Failed to fetch page body");return(await n.json()).data},enabled:!!i&&i!=="quickstart"&&v.some(n=>n.slug===i)}),l=o.useMemo(()=>v.length===0?y:v.map(n=>{const t=y.find(r=>r.id===n.slug);return{id:n.slug,title:n.title,level:2,body:n.slug===i&&(g!=null&&g.body)?g.body:(t==null?void 0:t.body)??""}}),[v,y,i,g]),S=o.useMemo(()=>{const n=h.trim().toLowerCase();return n?l.filter(t=>t.title.toLowerCase().includes(n)||t.body.toLowerCase().includes(n)):l},[l,h]),D=o.useMemo(()=>{const n=l.length+(u?1:0);if(n===0)return 0;let t=a.filter(r=>r==="quickstart"||l.some(s=>s.id===r)).length;return Math.round(t/n*100)},[l,a,u]),O=o.useMemo(()=>{const n=[];return a.includes("quickstart")&&n.push("quickstart"),m.includes("gis_intel")&&n.push("gis_intel"),m.includes("routine_plan")&&n.push("routine_plan"),l.length>0&&l.every(r=>a.includes(r.id))&&n.push("scholar"),n},[a,m,l]),_=n=>{let t;a.includes(n)?t=a.filter(r=>r!==n):t=[...a,n],x(t)},J=(n,t)=>{H(r=>({...r,[`${i}-${n}`]:t})),M(r=>({...r,[`${i}-${n}`]:!1}))},Z=(n,t)=>{let r=!0;if(t.questions.forEach((s,c)=>{I[`${n}-${c}`]!==s.correctAnswer&&(r=!1),M(k=>({...k,[`${n}-${c}`]:!0}))}),r){if(!m.includes(t.id)){const s=[...m,t.id];j(s);try{localStorage.setItem("vaxplan.quizzes.completed",JSON.stringify(s))}catch{}}a.includes(n)||x([...a,n])}};return e.jsxs("div",{className:"space-y-6",children:[e.jsx(P,{className:"border-indigo-500/20 bg-indigo-50/50 dark:bg-indigo-950/20",children:e.jsx(T,{className:"pt-6",children:e.jsxs("div",{className:"flex flex-col md:flex-row md:items-center justify-between gap-6",children:[e.jsxs("div",{className:"space-y-2 flex-1",children:[e.jsxs("div",{className:"flex items-center gap-2",children:[e.jsx(L,{className:"h-5 w-5 text-indigo-500"}),e.jsx("h2",{className:"font-bold text-base",children:"Your Learning Academy Progress"})]}),e.jsxs("div",{className:"flex items-center justify-between text-xs text-muted-foreground",children:[e.jsxs("span",{children:["Modules Read: ",a.length," of ",l.length+(u?1:0)]}),e.jsxs("span",{children:[D,"% Complete"]})]}),e.jsx(le,{value:D,className:"h-2 bg-muted-foreground/15"})]}),e.jsxs("div",{className:"border-t md:border-t-0 md:border-l border-indigo-500/10 pt-4 md:pt-0 md:pl-6",children:[e.jsxs("div",{className:"text-xs font-semibold text-indigo-500 uppercase tracking-wider mb-2 flex items-center gap-1.5",children:[e.jsx(pe,{className:"h-4 w-4"})," Unlocked Badges (",O.length," / ",Y.length,")"]}),e.jsx("div",{className:"flex gap-2 flex-wrap",children:Y.map(n=>{const t=O.includes(n.id);return e.jsxs("div",{title:`${n.name}: ${n.description} (${t?"Unlocked":"Locked"})`,className:`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs font-medium select-none transition-all duration-300 ${t?`bg-gradient-to-r ${n.color} text-white border-transparent shadow-sm scale-100 hover:scale-105`:"bg-muted text-muted-foreground/60 border-muted-foreground/15 opacity-60"}`,children:[e.jsx("span",{children:n.icon}),e.jsx("span",{children:n.name}),!t&&e.jsx(ee,{className:"h-3 w-3 ml-0.5 opacity-60"})]},n.id)})})]})]})})}),u&&e.jsxs(P,{className:"border-indigo-500/30 bg-gradient-to-br from-indigo-500/5 to-sky-500/5",children:[e.jsx(G,{className:"pb-3",children:e.jsxs("div",{className:"flex items-center justify-between gap-4 flex-wrap",children:[e.jsxs(F,{className:"flex items-center gap-2 text-base",children:[e.jsx(ne,{className:"h-5 w-5 text-indigo-500"}),"Facility Quick-Start",a.includes("quickstart")&&e.jsxs(E,{variant:"secondary",className:"ml-1 text-xs bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-0 flex items-center gap-0.5",children:[e.jsx(W,{className:"h-3 w-3"})," Completed"]})]}),e.jsxs("div",{className:"flex gap-2",children:[e.jsx(w,{size:"sm",variant:a.includes("quickstart")?"ghost":"default",className:a.includes("quickstart")?"text-muted-foreground text-xs":"text-xs font-semibold bg-indigo-600 text-white hover:bg-indigo-700",onClick:()=>{const n=a.includes("quickstart");x(n?a.filter(t=>t!=="quickstart"):[...a,"quickstart"])},children:a.includes("quickstart")?"Mark Unread":"Mark as Read"}),e.jsx(w,{size:"sm",variant:"outline",onClick:()=>window.print(),"data-testid":"btn-print-quickstart",children:"Print"})]})]})}),e.jsx(T,{children:e.jsx("article",{className:"prose prose-sm max-w-none dark:prose-invert",children:e.jsx(B,{remarkPlugins:[U],components:{img:({src:n,alt:t})=>e.jsx("img",{src:n,alt:t,className:"rounded-lg shadow-md max-h-96 object-cover cursor-zoom-in transition-transform hover:scale-[1.01]",onClick:()=>b(n||null)})},children:ge})})})]}),e.jsxs(P,{children:[e.jsx(G,{className:"pb-3 border-b",children:e.jsxs("div",{className:"flex items-center justify-between gap-4 flex-wrap",children:[e.jsxs("div",{className:"space-y-1",children:[e.jsxs(F,{className:"flex items-center gap-2 text-base",children:[e.jsx(te,{className:"h-5 w-5 text-indigo-500"}),"VaxPlan End-User Wiki Guide"]}),e.jsx("p",{className:"text-xs text-muted-foreground",children:"Live role-by-role training handbook. Read modules, submit quizzes, and earn badges."})]}),e.jsxs(w,{size:"sm",variant:"outline",className:"gap-1.5","data-testid":"btn-download-guide-pdf",onClick:async()=>{try{if((await fetch("/VaxPlan-User-Guide.pdf",{method:"HEAD"})).ok){const t=document.createElement("a");t.href="/VaxPlan-User-Guide.pdf",t.download="VaxPlan-User-Guide.pdf",t.click()}else window.print()}catch{window.print()}},children:[e.jsx(re,{className:"h-3.5 w-3.5"}),"Download PDF"]})]})}),e.jsxs(T,{className:"space-y-3 pt-4",children:[e.jsxs("div",{className:"relative",children:[e.jsx(ae,{"aria-hidden":"true",className:"absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground"}),e.jsx(ie,{id:"guide-search",placeholder:"Search guide pages...",value:h,onChange:n=>d(n.target.value),className:"pl-8 h-9 text-sm","data-testid":"input-guide-search"})]}),$&&S.length===0?e.jsx("div",{className:"py-8 text-center text-sm text-muted-foreground",children:"Loading wiki pages from database..."}):S.length===0?e.jsx("p",{className:"text-sm text-muted-foreground py-6 text-center",children:"No matching pages found."}):e.jsx(ce,{type:"single",collapsible:!0,value:i,onValueChange:f,children:S.map(n=>{const t=a.includes(n.id),r=ye[n.id],s=r&&m.includes(r.id);return e.jsxs(de,{value:n.id,children:[e.jsxs(ue,{className:"text-sm hover:no-underline text-left flex items-center justify-between",children:[e.jsxs("div",{className:"flex items-center gap-2",children:[t?e.jsx(W,{className:"h-4 w-4 text-emerald-500 shrink-0"}):e.jsx("div",{className:"h-4 w-4 border rounded-full shrink-0 border-muted-foreground/30"}),e.jsx("span",{children:n.title})]}),r&&e.jsx(E,{variant:"outline",className:`ml-2 text-[10px] uppercase font-semibold ${s?"bg-emerald-500/10 text-emerald-600 border-0":"bg-indigo-500/10 text-indigo-600 border-indigo-200"}`,children:s?"Quiz Passed ✅":"Quiz Available 📝"})]}),e.jsx(he,{className:"pt-2",children:i===n.id&&v.length>0&&g===void 0?e.jsx("div",{className:"py-4 text-center text-xs text-muted-foreground",children:"Loading page body..."}):e.jsxs("div",{className:"space-y-6",children:[e.jsx("article",{className:"prose prose-sm max-w-none dark:prose-invert overflow-x-auto [&_table]:block [&_table]:overflow-x-auto [&_table]:whitespace-nowrap [&_table]:max-w-full",children:e.jsx(B,{remarkPlugins:[U],components:{img:({src:c,alt:p})=>e.jsx("img",{src:c,alt:p,className:"rounded-lg shadow-md max-h-96 object-cover cursor-zoom-in transition-transform hover:scale-[1.01]",onClick:()=>b(c||null)})},children:n.body})}),r&&e.jsxs("div",{className:"border border-indigo-500/20 bg-indigo-50/20 dark:bg-indigo-950/10 rounded-lg p-4 mt-6 space-y-4",children:[e.jsxs("div",{className:"flex items-center gap-2",children:[e.jsx(oe,{className:"h-5 w-5 text-indigo-500"}),e.jsx("h4",{className:"font-bold text-sm text-foreground m-0",children:r.title})]}),e.jsx("div",{className:"space-y-4 divide-y divide-indigo-500/5",children:r.questions.map((c,p)=>{const k=`${n.id}-${p}`,z=I[k],R=Q[k],C=z===c.correctAnswer;return e.jsxs("div",{className:"pt-4 first:pt-0 space-y-2",children:[e.jsxs("p",{className:"text-xs font-semibold text-foreground",children:[p+1,". ",c.question]}),e.jsx("div",{className:"grid gap-2",children:c.options.map((K,A)=>e.jsx("button",{type:"button",disabled:s,onClick:()=>J(p,A),className:`text-left text-xs px-3 py-2 border rounded-md transition-all ${z===A?R?C?"bg-emerald-500/10 border-emerald-500 text-emerald-600 dark:text-emerald-400 font-medium":"bg-rose-500/10 border-rose-500 text-rose-600 dark:text-rose-400 font-medium":"bg-indigo-500/15 border-indigo-500 font-medium":"bg-background border-muted hover:bg-muted/40"}`,children:K},A))}),R&&e.jsxs("div",{className:`text-xs p-2 rounded ${C?"bg-emerald-500/5 text-emerald-600":"bg-rose-500/5 text-rose-600"}`,children:[e.jsx("strong",{children:C?"Correct!":"Incorrect."})," ",c.explanation]})]},p)})}),s?e.jsxs("div",{className:"flex items-center justify-center gap-1.5 py-1 text-emerald-500 text-xs font-semibold",children:[e.jsx(L,{className:"h-4 w-4"})," Quiz Completed successfully!"]}):e.jsx(w,{size:"sm",onClick:()=>Z(n.id,r),className:"w-full text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white",children:"Submit Quiz Answers"})]}),e.jsxs("div",{className:"flex justify-between items-center border-t pt-4 mt-6",children:[e.jsx("span",{className:"text-xs text-muted-foreground",children:t?"You read this page ✅":"Finished reading?"}),e.jsx(w,{size:"sm",variant:t?"outline":"default",onClick:()=>_(n.id),className:"text-xs",children:t?"Mark Unread":"Mark as Read"})]})]})})]},n.id)})})]})]}),q&&e.jsxs("div",{onClick:()=>b(null),className:"fixed inset-0 bg-black/80 z-[999] flex items-center justify-center p-4 cursor-zoom-out backdrop-blur-sm",children:[e.jsx("button",{type:"button",className:"absolute top-4 right-4 bg-white/10 hover:bg-white/20 text-white rounded-full p-2",onClick:()=>b(null),children:e.jsx(se,{className:"h-5 w-5"})}),e.jsx("img",{src:q,alt:"Expanded view",className:"max-w-full max-h-[90vh] object-contain rounded-lg shadow-2xl"})]})]})}export{Ce as U};
