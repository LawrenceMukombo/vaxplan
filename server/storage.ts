...
export interface IStorage {
  ...
  getProvince(tenantId: string, id: number): Promise<Province | undefined>;
  createProvince(tenantId: string, data: InsertProvince): Promise<Province>;
  updateProvince(tenantId: string, id: number, data: Partial<InsertProvince>): Promise<Province | undefined>;
  ...
  getDistrict(tenantId: string, id: number): Promise<District | undefined>;
  createDistrict(tenantId: string, data: InsertDistrict): Promise<District>;
  updateDistrict(tenantId: string, id: number, data: Partial<InsertDistrict>): Promise<District | undefined>;
  ...
  */
  ...
  getSupervisionVisits(tenantId: string, filters?: { facilityId?: number; microplanId?: number; status?: string; visitType?: string }): Promise<SupervisionVisit[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  */
  ...
  async updateProvince(tenantId: string, id: number, data: Partial<InsertProvince>): Promise<Province | undefined> {
    const { tenantId: _i, ...safe } = data as any;
    const [p] = await db
      .update(provinces)
      .set(safe)
  ...
  async updateDistrict(tenantId: string, id: number, data: Partial<InsertDistrict>): Promise<District | undefined> {
    const { tenantId: _i, ...safe } = data as any;
    const [d] = await db
      .update(districts)
      .set(safe)
  ...
  async getVillages(tenantId: string, districtId?: number, facilityId?: number): Promise<Village[]> {
    ...
    return await db
      .select({
        ...
        // boundary excluded — full GeoJSON polygon fetched only by getVillage()
  ...
  async getVillage(tenantId: string, id: number): Promise<Village | undefined> {
    ...
    */
    ...
    // Modified Code: Enrich the detailed village object with its latest census population count
    const [v] = await db
      .select({
  ...
  */
  ...
  async getMicroplans(tenantId: string): Promise<Microplan[]> {
    // PERFORMANCE: exclude `staffing` (large JSONB array) from the list.
    // The list view only renders summary fields. Full detail including staffing
    // is loaded by getMicroplan() when a user opens a specific plan.
    // Type assertion is safe: staffing is a nullable JSONB blob the list never reads.
    return await db
      .select({
        id: microplans.id,
        ...
        campaignAntigen: microplans.campaignAntigen,
        campaignTargetAge: microplans.campaignTargetAge,
        campaignScope: microplans.campaignScope,
  ...
  async getSessionPlans(tenantId: string, facilityId?: number): Promise<SessionPlan[]> {
    // PERFORMANCE: exclude large JSONB columns from the list view.
    // `geojson` is a GeoJSON polygon for the session geofence (can be very large).
    // `vaccinatedCounts` and `vaccineAdjustments` are per-antigen objects.
    // These are only needed on the session detail/edit view via getSessionPlan().
    // Type assertion is safe: excluded fields are nullable JSONB blobs the list never reads.
    return await db
      .select({
        id: sessionPlans.id,
        tenantId: sessionPlans.tenantId,
        ...
        transportMode: sessionPlans.transportMode,
        estimatedDuration: sessionPlans.estimatedDuration,
        targetPopulation: sessionPlans.targetPopulation,
        status: sessionPlans.status,
        approvalStatus: sessionPlans.approvalStatus,
        notes: sessionPlans.notes,
        humanResources: sessionPlans.humanResources,
        keyStakeholders: sessionPlans.keyStakeholders,
        planType: sessionPlans.planType,
        campaignAntigen: sessionPlans.campaignAntigen,
        campaignTargetAge: sessionPlans.campaignTargetAge,
        campaignScope: sessionPlans.campaignScope,
        teamType: sessionPlans.teamType,
        isAchieved: sessionPlans.isAchieved,
        outreachPurpose: sessionPlans.outreachPurpose,
        completedAt: sessionPlans.completedAt,
        // vaccineAdjustments, geojson, vaccinatedCounts excluded — fetched by getSessionPlan()
        createdAt: sessionPlans.createdAt,
        updatedAt: sessionPlans.updatedAt,
      })
      .from(sessionPlans)
      .where(withTenant(sessionPlans, tenantId, facilityId ? eq(sessionPlans.facilityId, facilityId) : undefined)) as unknown as SessionPlan[];
  }
  ...
  // --- Supervision visits ---
  async getSupervisionVisits(tenantId: string, filters?: { facilityId?: number; microplanId?: number; status?: string; visitType?: string }): Promise<SupervisionVisit[]> {
    const conds: any[] = [];
    ...
    if (filters?.visitType) conds.push(eq(supervisionVisits.visitType, filters.visitType));
    return await db
      .select()
  ...
  async listAuditLogs(tenantId: string, filters?: { userId?: string; entityType?: string; entityId?: string; limit?: number }): Promise<AuditLog[]> {
    ...
    if (filters?.entityId) conds.push(eq(auditLogs.entityId, parseInt(filters.entityId, 10)));
    return await db
      .select()
      .from(auditLogs)
