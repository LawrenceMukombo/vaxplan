...
export const tenantContext: RequestHandler = async (req, _res, next) => {
  ...
  // path sets claims.sub = dbUser.id, but if claims is missing we fall back.
  ...
  // NOTE: this block must come AFTER the super-admin viewTenantId override
  // above. Checking session.tenantId before the override would cause a super-
  // admin who has switched tenants to always fall back to their home tenant.
  if (req.session.tenantId) {
    req.tenantId = req.session.tenantId;
    return next();
  }
  try {
    // Same dual-path userId resolution as the override check above.
    const userId = (req.user as any)?.claims?.sub || (req.user as any)?.id;
