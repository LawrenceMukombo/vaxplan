        ...
        <TabsContent value="existing" className="space-y-3 mt-4">
          ...
          {plans.map((p) => (
            <Card key={p.id} data-testid={`card-annual-plan-${p.id}`}>
              ...
              {!!((p as any).priorities || (p as any).narrative || ((p as any).targetsByAntigen && Object.keys((p as any).targetsByAntigen).length > 0)) && (
                <CardContent className="text-sm space-y-2">
                  {(p as any).priorities && (
                    <div>
                      <div className="font-medium">Priorities</div>
                      <div className="text-muted-foreground whitespace-pre-wrap">{String((p as any).priorities)}</div>
                    </div>
                  )}

                  {!!(p as any).targetsByAntigen && Object.keys((p as any).targetsByAntigen).length > 0 && (
                    <div>
                      <div className="font-medium">Coverage targets</div>
                      <div className="flex flex-wrap gap-2 mt-1">
                        {Object.entries((p as any).targetsByAntigen).map(([k, v]) => (
                  ...
                  {!!(p as any).fundingMix && Object.keys((p as any).fundingMix).length > 0 && (
                    <div>
                      <div className="font-medium">Funding mix</div>
                      <div className="flex flex-wrap gap-2 mt-1">
                        {Object.entries((p as any).fundingMix).map(([k, v]) => (
                          <Badge key={k} variant="outline">{k}: {String(v)}%</Badge>
                        ))}
                      </div>
                    </div>
