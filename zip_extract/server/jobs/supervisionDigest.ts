...
/**
 * Schedules the weekly supervision digest to fire at midnight UTC every Monday.
 *
 * Previously used setInterval every 60 minutes (polling 168 times/week) with a
 * shouldRunNow() gate — 167 out of 168 polls were no-ops, still waking the
 * process every hour. scheduleAtMidnight({ dayOfWeek: 1 }) sleeps exactly until
 * Monday midnight UTC and does nothing on other nights.
 *
 * Set SUPERVISION_DIGEST_ENABLED=0 to disable completely.
 */
export function startSupervisionDigestScheduler(): void {
  if (process.env.SUPERVISION_DIGEST_ENABLED === "0") {
    console.log("[supervision-digest] disabled via SUPERVISION_DIGEST_ENABLED=0");
    return;
  }
  scheduleAtMidnight(
    "supervision-digest",
    async () => {
      const results = await runSupervisionDigest({ now: new Date() });
      const totals = results.reduce(
        (acc, r) => {
          acc.delivered += r.delivered;
          acc.overdue += r.totalOverdue;
      ...
      console.log(
        `[supervision-digest] weekly run — tenants=${results.length} delivered=${totals.delivered} overdueFacilities=${totals.overdue} errors=${totals.errors}`,
      );
    },
    { dayOfWeek: 1, offsetMinutes: 30 }, // Monday 00:30 UTC — staggered from other jobs
  );
}

export function stopSupervisionDigestScheduler(): void {
  cancelMidnightJob("supervision-digest");
}
