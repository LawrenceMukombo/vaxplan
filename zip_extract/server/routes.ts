...
*/
  ...
  app.post("/api/facilities/:facilityId/staff", ...auth, async (req: any, res) => {
    try {
      const facilityId = parseInt(req.params.facilityId);
      if (isNaN(facilityId)) {
        return res.status(400).json({ message: "Invalid facilityId" });
  ...
  app.patch("/api/facilities/:facilityId/staff/:staffId", ...auth, async (req: any, res) => {
    try {
  ...
  app.delete("/api/facilities/:facilityId/staff/:staffId", ...auth, async (req: any, res) => {
    try {
      const staffId = parseInt(req.params.staffId);
      const facilityId = parseInt(req.params.facilityId);
      if (isNaN(staffId) || isNaN(facilityId)) {
  ...
  app.post("/api/facilities/:facilityId/staff/bulk", ...auth, async (req: any, res) => {
    try {
  ...
  app.get("/api/facilities/:facilityId/hfc-committee", ...auth, async (req: any, res) => {
    try {
      const facilityId = parseInt(req.params.facilityId);
  ...
  app.post("/api/facilities/:facilityId/hfc-committee", ...auth, async (req: any, res) => {
    try {
      const facilityId = parseInt(req.params.facilityId);
      if (isNaN(facilityId)) return res.status(400).json({ message: "Invalid facilityId" });
  ...
  app.patch("/api/facilities/:facilityId/hfc-committee/:memberId", ...auth, async (req: any, res) => {
    try {
  ...
  app.delete("/api/facilities/:facilityId/hfc-committee/:memberId", ...auth, async (req: any, res) => {
    try {
      const memberId = parseInt(req.params.memberId);
      const facilityId = parseInt(req.params.facilityId);
      if (isNaN(memberId) || isNaN(facilityId)) return res.status(400).json({ message: "Invalid parameters" });
  ...
  app.get("/api/facilities/:facilityId/chvs", ...auth, async (req: any, res) => {
    try {
      const facilityId = parseInt(req.params.facilityId);
      if (isNaN(facilityId)) return res.status(400).json({ message: "Invalid facilityId" });
  ...
  app.post("/api/facilities/:facilityId/chvs", ...auth, async (req: any, res) => {
    try {
      const facilityId = parseInt(req.params.facilityId);
      if (isNaN(facilityId)) return res.status(400).json({ message: "Invalid facilityId" });
  ...
  app.patch("/api/facilities/:facilityId/chvs/:chvId", ...auth, async (req: any, res) => {
    try {
  ...
  app.delete("/api/facilities/:facilityId/chvs/:chvId", ...auth, async (req: any, res) => {
    try {
      const chvId = parseInt(req.params.chvId);
      const facilityId = parseInt(req.params.facilityId);
      if (isNaN(chvId) || isNaN(facilityId)) return res.status(400).json({ message: "Invalid parameters" });
  ...
  app.post("/api/facilities/:facilityId/chvs/bulk", ...auth, async (req: any, res) => {
    try {
      const facilityId = parseInt(req.params.facilityId, 10);
      if (isNaN(facilityId)) return res.status(400).json({ message: "Invalid facility id" });
      const parsed = parseBulkItems(req.body);
  ...
  async function estimateAndSaveVillagePopulation(
    tenantId: string,
    villageId: number,
    dbInstance: any = db
  ): Promise<number | null> {
  ...
  // Returns only the fields the dashboard needs: id, name, districtId,
  // assignedFacilityId, isHardToReach. For large tenants (20K+ villages) this
  // is ~23x smaller than the full record payload (no boundary/polygon/comments).
  ...
  */
  ...
  app.post("/api/villages", ...auth, async (req: any, res) => {
    try {
      ...
      // If a boundary polygon is provided, check for overlaps to prevent overlapping community polygons
      if (body.boundary) {
        const overlaps = await detectCommunityBoundaryOverlaps(req.tenantId, body.boundary);
        if (overlaps.length > 0) {
          const overlapNames = overlaps.map(o => o.villageName).join(", ");
          return res.status(400).json({
            message: `Overlap detected: The boundary overlaps with community: ${overlapNames}. Overlapping boundaries are not allowed.`
          });
        }
      }
      ...
      // Retrieve the enriched village object (which contains the population property)
      ...
      const overlaps = enrichedVillage && (enrichedVillage as any).boundary
        ? await detectCommunityBoundaryOverlaps(req.tenantId, (enrichedVillage as any).boundary, Number(enrichedVillage.id))
        : [];
      res.status(201).json({ ...(enrichedVillage || village), overlaps });
    } catch (error) {
      console.error("Error creating village:", error);
      res.status(400).json({ message: "Invalid village data" });
    }
        ...
        for (let j = 0; j < inserted.length; j++) {
          if (j % 10 === 0 || j === inserted.length) {
            extractionStatus.set(req.tenantId, {
              ...
              stage: `Estimating population for extracted communities (${j}/${inserted.length})...`
            });
          }
  ...
  */
  ...
  app.patch("/api/villages/:id", ...auth, async (req: any, res) => {
    try {
      ...
      // If a boundary polygon is provided, check for overlaps to prevent overlapping community polygons
      if (body.boundary) {
        const overlaps = await detectCommunityBoundaryOverlaps(req.tenantId, body.boundary, entityId);
        if (overlaps.length > 0) {
          const overlapNames = overlaps.map(o => o.villageName).join(", ");
          return res.status(400).json({
            message: `Overlap detected: The boundary overlaps with community: ${overlapNames}. Overlapping boundaries are not allowed.`
      ...
      const overlaps = enrichedVillage && (enrichedVillage as any).boundary
        ? await detectCommunityBoundaryOverlaps(req.tenantId, (enrichedVillage as any).boundary, entityId)
        : [];
  ...
  */
  ...
  app.get("/api/microplans", ...auth, async (req: any, res) => {
    try {
      ...
      // buildScopedFilters resolves the full set of facilityIds for this user
      // in 2 DB round-trips max (once), then recordInGeoScope is O(1) per plan.
  ...
  */
  ...
  app.post("/api/clients/:id/vaccinate-batch", isAuthenticated, requireTenant, async (req: any, res) => {
    try {
  ...
  function invalidateTenantIndicatorCache(tenantId: string): void {
...
3. Support Markdown formatting for clear tabular data or bulleted highlights.`;
  ...
  // Returns a printable standalone HTML page embedding the OpenStreetMap tiles.
