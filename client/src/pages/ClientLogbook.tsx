...
// Premium Offline-Available Vector Pin Icon (Built from shared SVG constants)
...
// Added helper to calculate if a vaccine is missed under WHO/UNICEF/GAVI cohort and safety limits.
// OPV 0 is missed after 28 days.
// Rotavirus 1 is missed after 15 weeks (105 days).
// Rotavirus 2 & 3 are missed after 24 weeks (168 days).
// Infant primary series (BCG, OPV 1-3, Penta 1-3, PCV 1-3, IPV 1, MR 1, IPV 2) are missed after 12 months (365 days) for the cohort.
// MR 2 is missed after 24 months (730 days).
...
const getAICatchUpPlan = (client: Client, vaccinations: ClientVaccination[]) => {
  ...
  */
  ...
  const plan: Array<{
    antigen: string;
    status: "overdue" | "due_soon" | "up_to_date" | "missed";
    dueDate: Date;
    advisory: string;
  }> = [];
  ...
  series.forEach((s) => {
    ...
    let status: "overdue" | "due_soon" | "up_to_date" | "missed" = "up_to_date";
    let advisory = "";
    const weeksDiff = Math.floor((today.getTime() - computedDueDate.getTime()) / (7 * 24 * 3600 * 1000));
    const ageDays = (today.getTime() - dob.getTime()) / (24 * 3600 * 1000);

    if (today > computedDueDate) {
      const isMissed = isVaccineMissed(nextDose.code, ageDays);
      
      if (isMissed && (nextDose.code === "OPV_0" || nextDose.code.startsWith("ROTA"))) {
        status = "missed";
        const limitStr = nextDose.code === "OPV_0" ? "4 weeks" : nextDose.code === "ROTA_1" ? "15 weeks" : "24 weeks";
        advisory = `${nextDose.name} is missed (upper limit of ${limitStr} exceeded). Do NOT administer.`;
      } else if (isMissed) {
        // Clinically eligible for catch-up, but missed infant cohort target age (e.g. child > 12 months)
        status = "overdue";
        const limitStr = nextDose.code === "MR_2" ? "24 months" : "12 months";
        advisory = `${nextDose.name} is missed for the routine cohort (exceeded ${limitStr}), but catch-up vaccination is recommended. Administer today.`;
        if (nextDoseIdx < s.doses.length - 1) {
          const followUp = s.doses[nextDoseIdx + 1];
          const nextVisitDate = new Date(today.getTime() + 4 * 7 * 24 * 3600 * 1000);
          advisory += ` Then, schedule ${followUp.name} in exactly 4 weeks (${nextVisitDate.toLocaleDateString()}).`;
...
const getVaccinationDueStatus = (client: any, vaccinations: any[]) => {
  const catchUpPlan = getAICatchUpPlan(client, vaccinations || []);
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const nextWeek = new Date(today.getTime() + 7 * 24 * 60 * 60 * 1000);
  
  const dueToday: typeof catchUpPlan = [];
  const dueNextWeek: typeof catchUpPlan = [];

  catchUpPlan.forEach(p => {
...
const getAntigenStatus = (config: any, client: any, vaccinations: any[]) => {
  ...
  // 2. Check if due today or due later
  if (client.clientType === "pregnant_woman") {
    const parsedTt = vaccinations
      .filter((v: any) => v.vaccineName?.toUpperCase().includes("TT") || v.vaccineName?.toUpperCase().includes("TD"))
      .sort((a: any, b: any) => new Date(a.administeredDate).getTime() - new Date(b.administeredDate).getTime());
    
    const numGiven = parsedTt.length;
    const name = config.name.toUpperCase();
    let configIndex = 0;
    if (name.includes("2")) configIndex = 1;
    else if (name.includes("3")) configIndex = 2;
    else if (name.includes("4")) configIndex = 3;
    else if (name.includes("5")) configIndex = 4;

    if (configIndex < numGiven) {
      return { status: "given", disabled: true, label: "Given" };
    }

    if (configIndex > numGiven) {
      return { status: "later", disabled: true, label: "Seq Pending" };
    }

    if (numGiven === 0) {
      return { status: "due", disabled: false, label: "Due Today" };
    } else {
      const prevDose = parsedTt[numGiven - 1];
      const prevDate = new Date(prevDose.administeredDate);
      let gapMs = 4 * 7 * 24 * 60 * 60 * 1000; // 4 weeks
      if (numGiven === 2) gapMs = 26 * 7 * 24 * 60 * 60 * 1000; // 6 months
      else if (numGiven >= 3) gapMs = 52 * 7 * 24 * 60 * 60 * 1000; // 1 year

      const nextDueDate = new Date(prevDate.getTime() + gapMs);
      ...
      if (today < nextDueDate) {
        return { 
          status: "later", 
          disabled: true, 
          label: `Due ${nextDueDate.toLocaleDateString()}` 
        };
      } else {
        return { status: "due", disabled: false, label: "Due Today" };
      }
    ...
    // Child client
    const dob = new Date(client.dateOfBirth);
    const recommendedWeeks = config.recommendedAgeWeeks ?? 0;
    const dueDate = new Date(dob.getTime() + recommendedWeeks * 7 * 24 * 60 * 60 * 1000);
    dueDate.setHours(0,0,0,0);

    const nameNorm = config.name.toUpperCase();
    let seriesName = "";
    let doseNum = 1;

    if (nameNorm.includes("PENTAVALENT") || nameNorm.includes("PENTA")) {
      seriesName = "PENTA";
      if (nameNorm.includes("2")) doseNum = 2;
      else if (nameNorm.includes("3")) doseNum = 3;
    } else if (nameNorm.includes("OPV")) {
      seriesName = "OPV";
      if (nameNorm.includes("1")) doseNum = 1;
      else if (nameNorm.includes("2")) doseNum = 2;
      else if (nameNorm.includes("3")) doseNum = 3;
      else if (nameNorm.includes("0")) doseNum = 0;
    } else if (nameNorm.includes("PCV")) {
      seriesName = "PCV";
      if (nameNorm.includes("2")) doseNum = 2;
      else if (nameNorm.includes("3")) doseNum = 3;
    } else if (nameNorm.includes("ROTAVIRUS") || nameNorm.includes("ROTA")) {
      seriesName = "ROTA";
      if (nameNorm.includes("2")) doseNum = 2;
      else if (nameNorm.includes("3")) doseNum = 3;
    } else if (nameNorm.includes("IPV")) {
      seriesName = "IPV";
      if (nameNorm.includes("2")) doseNum = 2;
    } else if (nameNorm.includes("MEASLES-RUBELLA") || nameNorm.includes("MR")) {
      seriesName = "MR";
      if (nameNorm.includes("2")) doseNum = 2;
    }

    if (seriesName && doseNum > 1) {
      const hasPrev = vaccinations.some(v => {
        const vName = v.vaccineName?.toUpperCase() || "";
        return vName.includes(seriesName) && vName.includes(String(doseNum - 1));
      });

      if (!hasPrev) {
        return { status: "later", disabled: true, label: `Req ${seriesName} ${doseNum - 1}` };
      }

      const prevDoseRecord = vaccinations.find(v => {
        const vName = v.vaccineName?.toUpperCase() || "";
        return vName.includes(seriesName) && vName.includes(String(doseNum - 1));
      });
      if (prevDoseRecord) {
        const prevDate = new Date(prevDoseRecord.administeredDate);
        const minGapDate = new Date(prevDate.getTime() + 4 * 7 * 24 * 60 * 60 * 1000);
        minGapDate.setHours(0,0,0,0);
        if (today < minGapDate) {
          return { status: "later", disabled: true, label: `Gap due ${minGapDate.toLocaleDateString()}` };
        }
      }
    }
...
export default function ClientLogbook() {
  ...
  const [preselectedVaccineName, setPreselectedVaccineName] = useState<string | undefined>(undefined);
  const [isVillageSelectOpen, setIsVillageSelectOpen] = useState(false);
  // Original Code preserved for reference:
  // const [gridRows, setGridRows] = useState<any[]>([]);
  const [gridRows, setGridRows] = useState<any[]>([]);
  // Added visibleDueQueueCount state to paginate/limit dueQueue list rendering for better performance
  const [visibleDueQueueCount, setVisibleDueQueueCount] = useState(10);

  
  // Collapsible GPS capture and interactive Mini-map coordinates picker states
  ...
  // Geo states for filtering registry table
  ...
  // Admin hierarchy queries (required for activeFacilityId and geofilters)
  ...
  const { data: clients, isLoading: loadingClients } = useQuery<Client[]>({
    ...
    queryFn: async ({ queryKey }) => {
      const [_key, filters] = queryKey as [string, { facilityId: number | null, districtId: number | null, provinceId: number | null }];
      if (!navigator.onLine) {
        let all = (await offlineDb.clients.toArray()) as unknown as Client[];
        if (filters.facilityId) {
          all = all.filter(c => c.facilityId === filters.facilityId);
        }
        return all;
      }
      const params = new URLSearchParams();
      if (filters.facilityId) params.append("facilityId", String(filters.facilityId));
      if (filters.districtId) params.append("districtId", String(filters.districtId));
      if (filters.provinceId) params.append("provinceId", String(filters.provinceId));
  ...
  // Villages were fetched regardless of whether any facility/district filters were active,
  // causing full-dataset downloads (21k+ records) for national administrators.
  const { data: villages } = useQuery<Village[]>({
    queryKey: ["/api/villages", { facilityId: activeFacilityId, districtId: geoFilterDistrictId }],
    queryFn: async ({ queryKey }) => {
      const [_key, filters] = queryKey as [string, { facilityId: number | null, districtId: number | null }];
      if (!navigator.onLine) {
        let all = (await offlineDb.villages.toArray()) as unknown as Village[];
        if (filters.facilityId) {
          all = all.filter(v => v.assignedFacilityId === filters.facilityId);
        } else if (filters.districtId) {
          all = all.filter(v => v.districtId === filters.districtId);
        }
        return all;
      }
      const params = new URLSearchParams();
      if (filters.facilityId && filters.facilityId > 0) {
        params.append("facilityId", String(filters.facilityId));
      } else if (filters.districtId && filters.districtId > 0) {
  ...
  */
  ...
  // Updated Code:
  // Only execute the villages lookup query if a facility or district scope is active.
  // This prevents national admins from triggering full-replica 11MB downloads.
  const { data: villages } = useQuery<Village[]>({
    queryKey: ["/api/villages", { facilityId: activeFacilityId, districtId: geoFilterDistrictId }],
    queryFn: async ({ queryKey }) => {
      const [_key, filters] = queryKey as [string, { facilityId: number | null, districtId: number | null }];
    ...
    enabled: activeFacilityId > 0 || (geoFilterDistrictId !== null && geoFilterDistrictId > 0),
  });
  ...
  const vaccinateBatchMutation = useMutation({
    mutationFn: async ({ clientId, data }: { clientId: string; data: any[] }) => {
      if (!navigator.onLine) {
        const results = [];
      ...
      return apiRequest("POST", `/api/clients/${clientId}/vaccinate-batch`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/clients"] });
      if (selectedClient) {
      ...
      setIsVaccinateOpen(false);
      setVaccinateClient(null);
      setPreselectedVaccineName(undefined);
      toast({
        title: navigator.onLine ? "Immunizations Logged" : "Immunizations Queued Offline",
        description: navigator.onLine 
          ? "Doses successfully registered with batch & VVM telemetry."
          : "Saved locally. Immunizations will sync automatically once internet is restored.",
      });
    },
    onError: (err: any) => {
      toast({
        title: "Failed to Log Doses",
        description: err.message,
        variant: "destructive",
      });
    },
  ...
  const sendReminderMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      // The backend logs the SMS dispatch; we no longer open the local messaging app.
      toast({
  ...
  */
  ...
  const handleShareSubmit = async () => {
    ...
    try {
      ...
      // External app launching code removed. The system now dispatches these notifications silently via backend APIs/gateways.
        ...
        description: (
          <div className="space-y-1.5 text-xs pt-1">
            <p>
              ...
              <strong>{shareMethod === "email" ? "Secure Email Attachment" : shareMethod === "sms" ? `SMS Carrier Gateway (${messagingSenderNumber})` : `WhatsApp API Portal (${messagingSenderNumber})`}</strong>.
            </p>
            {res.attachment && (
  ...
  */
  ...
  // Original duplicate declarations commented out to resolve ReferenceErrors (moved to top of component):
  /*
  const [adminProvinceId, setAdminProvinceId] = useState<number | null>(null);
  const [adminDistrictId, setAdminDistrictId] = useState<number | null>(null);
  const [adminFacilityId, setAdminFacilityId] = useState<number | null>(null);
  ...
  */
  ...
  const createVillageMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Original Code commented out for backward-compatibility:
      // Set the newly created village as the selected one ONLY on the registration form.
      // If the user was editing a client, the selection was lost, preventing the edit form from saving.
      /*
      clientForm.setValue("villageId", data.id);
      */
      
      // Updated Code:
      // Set the newly created village as the selected one on both the registration and edit forms
      // so that whichever modal is active gets the newly created village ID.
      clientForm.setValue("villageId", data.id);
      editForm.setValue("villageId", data.id);
  ...
  */
  ...
  // Updated Code:
  // Resolves the village name by checking pre-resolved geo fields first before querying
  // the (possibly unloaded) villages dictionary array.
  const getVillageName = (villageId: number, client?: any) => {
    return client?._geoVillageName || villages?.find((v) => v.id === villageId)?.name || `Village #${villageId}`;
  };
    ...
    */
    ...
    // or a specialized Cross-Border/Foreign Country badge.
    {
      ...
      render: (item: Client) => {
        ...
        const vName = getVillageName(item.villageId, clientItem);
        const path = (clientItem._geoDistrictName && clientItem._geoProvinceName)
          ? `${clientItem._geoDistrictName} District, ${clientItem._geoProvinceName} Province`
          : getVillageResidencyPath(item.villageId);
        return (
    ...
    },
    {
      ...
      render: (item: Client) => {
        const { dueToday, dueNextWeek } = getVaccinationDueStatus(item, (item as any).vaccinations || []);
        
        const handleAdministerClick = (e: React.MouseEvent, vaccineName?: string) => {
          e.stopPropagation();
          ...
          return (
            <div className="flex items-center justify-between gap-2 min-w-[150px]">
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap gap-1">
                  {dueToday.map(p => (
                    <button
                      key={p.antigen}
                      onClick={(e) => handleAdministerClick(e, p.antigen)}
                      className="text-[10px] text-red-500 hover:text-red-700 underline font-semibold bg-red-500/5 hover:bg-red-500/10 px-1 py-0.5 rounded transition-all"
                    >
                      {p.antigen}
                    </button>
                  ))}
              ...
              <Button
                ...
                onClick={(e) => handleAdministerClick(e)}
                title="Administer Vaccines"
              >
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-bold text-amber-600 dark:text-amber-400">
                    {dueNextWeek.length} due in 1 week
                  </span>
                </div>
                <div className="flex flex-wrap gap-1">
                  {dueNextWeek.map(p => (
                    <button
                      key={p.antigen}
                      onClick={(e) => handleAdministerClick(e, p.antigen)}
                      className="text-[10px] text-amber-600 hover:text-amber-800 underline font-semibold bg-amber-500/5 hover:bg-amber-500/10 px-1 py-0.5 rounded transition-all"
                    >
                      {p.antigen}
                    </button>
                  ))}
          ...
          <div className="flex items-center justify-between gap-2 min-w-[150px]">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-bold text-green-600 dark:text-green-400">
                No vaccine due in 1 week
              </span>
            </div>
            <Button
              variant="ghost"
              size="icon"
              className="h-7 w-7 text-green-500 hover:text-green-600 hover:bg-green-500/10 shrink-0"
              onClick={(e) => handleAdministerClick(e)}
              title="Administer Vaccines"
            >
              <Syringe className="h-3.5 w-3.5" />
          ...
          <div className="grid gap-6">
            ...
            {dueQueue.length === 0 ? (
              ...
              <div className="grid gap-4">
                {/* Original Code: mapped over entire dueQueue. Commented out to implement list pagination.
                {dueQueue.map(({ client, weeksOld, dueVaccines }) => (
                */}
                {dueQueue.slice(0, visibleDueQueueCount).map(({ client, weeksOld, dueVaccines }) => (
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-y-1 gap-x-4 text-xs text-muted-foreground">
                          <div className="flex items-center gap-1">
                            ...
                            <span>{getVillageName(client.villageId, client)}</span>
                          </div>
                          {client.parentName && (
                            <div className="flex items-center gap-1">
            ...
            {dueQueue.length > visibleDueQueueCount && (
              <div className="flex justify-center pt-4">
                <Button
                  variant="outline"
                  onClick={() => setVisibleDueQueueCount(prev => prev + 10)}
                  className="gap-2 border-primary/20 hover:bg-primary/5 text-primary"
                >
                  Load More ({dueQueue.length - visibleDueQueueCount} remaining)
                </Button>
              </div>
            )}
          </div>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-card border border-border text-foreground rounded-3xl shadow-2xl p-6 font-sans">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold flex items-center gap-2 text-foreground">
              ...
              Batch Log Immunizations
            </DialogTitle>
          </DialogHeader>
          
          {vaccinateClient && (
            <div className="flex flex-col h-full max-h-[70vh]">
              <div className="bg-muted/50 p-3.5 rounded-2xl text-sm mb-4 border border-border/60 flex items-center justify-between">
                <div>
                  <p className="font-extrabold text-foreground">Recipient: {vaccinateClient.name}</p>
              ...
              <div className="overflow-x-auto border border-border/80 rounded-2xl flex-1 max-h-[400px]">
                <table className="w-full text-sm text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-border/60">
                    {gridRows.map((row, idx) => {
                      const isDisabled = row.statusInfo.disabled;
                      const isGiven = row.statusInfo.status === "given";
                      const isLater = row.statusInfo.status === "later";
                        ...
                        >
                          <td className="p-3 text-center">
                            <Checkbox
                              ...
                              disabled={isDisabled}
                              onCheckedChange={(checked) => {
                                setGridRows(prev => prev.map((r, i) => i === idx ? { ...r, checked: !!checked } : r));
                              }}
                            />
                          </td>
                          ...
                          <td className="p-3">
                            <Badge 
                              className={`text-[10px] px-2 py-0.5 rounded-full border-none font-extrabold ${
                                isGiven 
                          ...
                          <td className="p-3">
                            <Input
                              ...
                              onChange={(e) => {
                                setGridRows(prev => prev.map((r, i) => i === idx ? { ...r, administeredDate: e.target.value } : r));
                              }}
                          ...
                          <td className="p-3">
                            ...
                            >
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="2">Stage 2: Good</SelectItem>
                                <SelectItem value="3">Stage 3: Exceeded</SelectItem>
                                <SelectItem value="4">Stage 4: Blown</SelectItem>
                              </SelectContent>
                            </Select>
                          </td>
                        </tr>
              ...
              <div className="flex justify-end gap-2.5 pt-6 mt-4 border-t border-border/80">
                <Button 
                  variant="outline" 
                  type="button" 
                  onClick={() => setIsVaccinateOpen(false)}
                ...
                <Button 
                  ...
                  onClick={() => {
                    ...
                    const payload = selectedRows.map(r => ({
                      ...
                      vvmStatus: r.vvmStatus ? parseInt(r.vvmStatus) : null,
                    }));
                    vaccinateBatchMutation.mutate({ clientId: vaccinateClient.id, data: payload });
                  }}
                  className="rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold"
                >
                  {vaccinateBatchMutation.isPending ? "Logging Doses..." : "Log Selected Doses"}
                </Button>
              </div>
            </div>
          )}
          ...
          */}
          ...
          {selectedClient && (() => {
            ...
            */
            ...
            // New WHO/UNICEF/GAVI compliant status calculation
            const getDoseStatus = (dose: typeof VACCINE_SCHEDULE[0]) => {
              const matchingVac: any = findVaccination(dose.code, dose.name);
              if (matchingVac) {
                return {
            ...
            return (
              <div className="space-y-6 pt-2 vax-card-print-area">
                ...
                {(() => {
                  ...
                  const missedDoses = catchUpPlan.filter(p => p.status === "missed");
                  ...
                  return (
                    <div className="no-print relative overflow-hidden bg-radial from-slate-900/10 via-slate-900/5 to-transparent dark:from-indigo-500/10 dark:via-indigo-500/5 dark:to-transparent border border-indigo-500/30 dark:border-indigo-500/20 backdrop-blur-xl rounded-3xl p-6 shadow-xl shadow-indigo-500/5 animate-in fade-in duration-500 ring-1 ring-white/10">
                      ...
                      {missedDoses.length > 0 && (
                        <div className="mt-4 p-4 bg-slate-500/5 border border-rose-500/30 dark:border-rose-500/20 rounded-2xl space-y-2">
                          <h5 className="text-[11px] font-black uppercase tracking-wider text-rose-500 flex items-center gap-1.5 pl-1">
                            <AlertTriangle className="h-3.5 w-3.5 text-rose-500 shrink-0 animate-pulse" />
                            Missed Doses (Clinically Ineligible for Catch-up)
                          </h5>
                          <div className="space-y-2">
                            {missedDoses.map((dose, idx) => (
                              <div key={idx} className="text-xs text-muted-foreground flex justify-between gap-4 border-b border-border/40 pb-2 last:border-0 last:pb-0">
                                <span className="leading-relaxed"><strong>{dose.antigen}</strong>: {dose.advisory}</span>
                                <span className="text-[9px] font-black uppercase text-rose-500 bg-rose-500/10 border border-rose-500/20 px-1.5 py-0.5 rounded-md h-fit shrink-0 select-none">Ineligible</span>
                              </div>
                            ))}
                          </div>
                        </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          <div className="bg-indigo-50/50 dark:bg-indigo-950/20 p-2.5 border border-indigo-200/50 dark:border-indigo-500/10 rounded-xl col-span-2 print:bg-none print:border-black">
                                  ...
                                  <td className="py-1.5 px-1 text-center">
                                    ...
                                    */}
                                    {res.status === "administered" ? (
                                      ...
                                      <Badge className="bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-800/30 font-bold py-0.5 px-1.5 text-[8px] uppercase tracking-wide rounded-md flex items-center gap-0.5 justify-center">
                                        <AlertTriangle className="h-2 w-2 shrink-0 text-rose-500" /> MISSED
                                      </Badge>
                                    ) : res.status === "overdue" ? (
                                      <Badge className="bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-800/30 font-bold py-0.5 px-1.5 text-[8px] uppercase tracking-wide rounded-md flex items-center gap-0.5 justify-center">
                                        <AlertTriangle className="h-2 w-2 shrink-0 text-rose-500" /> OVERDUE
                                      </Badge>
                                    ) : res.status === "due" ? (
                                      <Badge className="bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-800/30 font-bold py-0.5 px-1.5 text-[8px] uppercase tracking-wide rounded-md flex items-center gap-0.5 justify-center">
                                  ...
                                  <td className="py-1.5 px-1">
                                    ...
                                    */}
                                    {res.status === "administered" ? (
                                      ...
                                      <div className="leading-tight text-rose-700 dark:text-rose-400">
                                        <span className="font-bold">Missed</span>
                                        <span className="block text-[7px] text-muted-foreground/80">(Limit exceeded)</span>
                                      </div>
                                    ) : res.status === "overdue" ? (
                                      <div className="leading-tight text-rose-600 dark:text-rose-400">
                                        <span className="font-bold">Due: {res.dueDate}</span>
