...
/** Shows pending/stuck outbox counts and lets the user retry or discard stuck items. */
function OutboxDiagnostics() {
  ...
  const reload = async () => {
    ...
    try {
      ...
      const all = tid
        ? await offlineDb.outbox.where("tenantId").equals(tid).toArray()
  ...
  useEffect(() => { reload(); }, [user?.tenantId]);

  const pending = items.filter((i) => i.retries < MAX_R);
  const stuck   = items.filter((i) => i.retries >= MAX_R);
  ...
  const handleClearFailed = async () => {
    try {
      ...
      await offlineDb.transaction("rw", offlineDb.outbox, async () => {
        for (const item of stuck) {
          if (item.id !== undefined) await offlineDb.outbox.delete(item.id);
        }
      });
      toast({ title: "Failed items discarded", description: `${stuck.length} item(s) removed from the outbox.`, variant: "destructive" });
      await reload();
    } catch (e: any) {
      toast({ title: "Error", description: e?.message, variant: "destructive" });
    }
  ...
  if (loading) return <p className="text-xs text-muted-foreground">Loading outbox…</p>;

  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <p className="font-medium text-sm">Sync Outbox</p>
        <div className="flex gap-1.5">
          <Badge variant="outline" className="text-xs">{pending.length} queued</Badge>
          {stuck.length > 0 && (
      ...
      {stuck.length > 0 && (
        <>
          <div className="rounded-md border border-orange-500/20 bg-orange-500/5 p-3 space-y-1 max-h-48 overflow-y-auto">
            <p className="text-xs font-semibold text-orange-400 mb-1.5">Failed items (max retries reached)</p>
            {stuck.map((item, i) => (
              <div key={i} className="text-xs text-muted-foreground truncate">
                <span className="font-mono text-orange-300">{item.method} {item.entityType}</span>
                {item.lastError && <span className="ml-2 opacity-70">— {item.lastError}</span>}
              </div>
            ))}
          </div>
          <div className="flex gap-2">
            <Button size="sm" variant="outline" className="text-xs h-7" onClick={handleRetry} data-testid="button-retry-failed">
              ↺ Retry failed
            </Button>
            <Button size="sm" variant="outline" className="text-xs h-7 text-destructive border-destructive/40" onClick={handleClearFailed} data-testid="button-clear-failed">
              Discard failed
            </Button>
          </div>
        </>
      ...
      {stuck.length === 0 && pending.length === 0 && (
        <p className="text-xs text-muted-foreground">Outbox is empty — all changes are synced.</p>
      )}
    </div>
  );
}
...
export default function Settings() {
  ...
  const toggleModule = (key: string) => {
    const updatedSettings = {
      ...
      modules: {
        ...modules,
        [key]: modules[key as keyof typeof DEFAULT_MODULES] === false ? true : false,
      },
    };
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-6">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        // 1. Delete the VaxPlan IndexedDB database (Dexie)
                        await new Promise<void>((resolve) => {
                          const req = indexedDB.deleteDatabase("VaxPlanOfflineDB");
                          req.onsuccess = () => resolve();
                          req.onerror = () => resolve(); // still continue
                          req.onblocked = () => resolve(); // unblock + continue
                        });
                        // 2. Clear VaxPlan localStorage keys
                        Object.keys(localStorage)
                          .filter((k) => k.startsWith("vaxplan"))
                          .forEach((k) => localStorage.removeItem(k));
                        toast({
                          title: "Cache cleared",
                          description: "Offline data removed. Reloading page…",
                        });
                ...
                <Separator />
                {/* Outbox diagnostics — shows stuck/failed queued items */}
                <OutboxDiagnostics />
              ...
              <CardContent>
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-xs">{formatBuildTime()}</span>
                  </div>
                  <div className="flex justify-between">
        ...
        <TabsContent value="communications" className="space-y-6 mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-6">
              {isNationalAdmin && <TenantCommunicationCard />}
            </div>
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">Gateway Setup Guide</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4 text-sm text-muted-foreground">
                  <p>
                    VaxPlan's Unified Communication Engine dynamically dispatches reminders, reports, and OTPs via the configured gateways.
                  </p>
                  <ul className="list-disc pl-4 space-y-2">
                    <li><strong>Email:</strong> Requires an SMTP app password from Google/Office365.</li>
                    <li><strong>SMS:</strong> Twilio or Africa's Talking API keys are required.</li>
                    <li><strong>WhatsApp:</strong> Ensure your Twilio number is approved for WhatsApp Business APIs.</li>
                  </ul>
                  <p className="pt-2 text-xs border-t border-border/40 mt-4">
                    Without configuration, the engine will use Mock / Console logging.
                  </p>
                </CardContent>
              </Card>
            </div>
        ...
        <TabsContent value="access" className="space-y-6 mt-6">
          ...
          <Card className="border border-border/80 shadow-xl bg-card/60 backdrop-blur-md overflow-hidden rounded-3xl">
            <CardHeader className="border-b border-border/40 pb-5">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-2xl bg-gradient-to-tr from-indigo-500/20 to-sky-500/20 dark:from-indigo-500/10 dark:to-sky-500/10 border border-indigo-500/30 dark:border-indigo-500/20 flex items-center justify-center">
                  <SettingsIcon className="h-5 w-5 text-indigo-600 dark:text-indigo-400" />
                </div>
                <div>
                  <CardTitle className="text-lg font-bold tracking-tight">System Modules Dashboard</CardTitle>
                  <CardDescription className="text-sm">
                    Enable or disable functional modules dynamically across sidebar navigation and page routings.
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-8 p-6">
              {MODULE_CATEGORIES.map((category) => {
                const categoryModules = MODULE_METADATA.filter((m) => m.category === category.id);
                return (
                  <div key={category.id} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {categoryModules.map((module) => {
                        ...
                        const isEnabled = modules[module.key as keyof typeof DEFAULT_MODULES] !== false;
                        return (
                          <div
                            key={module.key}
                            className="flex items-start justify-between p-4 rounded-2xl border border-border/50 bg-card/40 dark:bg-card/20 hover:bg-accent/40 dark:hover:bg-accent/10 transition-all duration-300 group"
                          >
                            <div className="flex gap-3 min-w-0 pr-4">
                              ...
                              <div className="space-y-1 min-w-0">
                                <Label className="text-sm font-semibold cursor-pointer select-none truncate block group-hover:text-foreground transition-colors">
                                  {module.title}
                                </Label>
                            ...
                            <div className="flex items-center self-center shrink-0">
                              <Switch
                                ...
                                className="data-[state=checked]:bg-gradient-to-r data-[state=checked]:from-indigo-600 data-[state=checked]:to-sky-600 dark:data-[state=checked]:from-indigo-500 dark:data-[state=checked]:to-sky-500"
                              />
                            </div>
                          </div>
                        );
