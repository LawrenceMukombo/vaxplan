...
const formSchema = z.object({
  ...
  if (data.malePopulation !== null && data.malePopulation !== undefined &&
      data.femalePopulation !== null && data.femalePopulation !== undefined) {
    return data.malePopulation + data.femalePopulation === data.totalPopulation;
  }
  return true;
}, {
  message: "Male and female population sum must equal total population",
  path: ["femalePopulation"],
});
  ...
  */
  ...
  useEffect(() => {
    if (editData) {
      ...
      // Disable auto-calculate when editing to preserve manual overrides
  ...
  const handleAutoCalculateChange = (checked: boolean) => {
    ...
    if (checked) {
      const total = form.getValues("totalPopulation") || 0;
      const ratios = getDemographicsRatios();
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormControl>
                      <Input 
                        ...
                        onChange={(e) => {
                          ...
                          if (autoCalculate) {
                            const ratios = getDemographicsRatios();
                            form.setValue("under1Population", Math.round(val * ratios.under1), { shouldValidate: true });
                            form.setValue("under5Population", Math.round(val * ratios.under5), { shouldValidate: true });
                            form.setValue("pregnantWomen", Math.round(val * ratios.pregnant), { shouldValidate: true });
                          }

                          const male = form.getValues("malePopulation");
                          const female = form.getValues("femalePopulation");
                          if (male !== null && male !== undefined) {
                            form.setValue("femalePopulation", Math.max(0, val - male), { shouldValidate: true });
                          } else if (female !== null && female !== undefined) {
                            form.setValue("malePopulation", Math.max(0, val - female), { shouldValidate: true });
                          }
                        }}
                    ...
                    <FormControl>
                      <Input 
                        ...
                        onChange={(e) => {
                          ...
                          const total = form.getValues("totalPopulation");
                          if (total && val !== null) {
                            form.setValue("femalePopulation", Math.max(0, total - val), { shouldValidate: true });
                          }
                        }}
                    ...
                    <FormControl>
                      <Input 
                        ...
                        onChange={(e) => {
                          ...
                          const total = form.getValues("totalPopulation");
                          if (total && val !== null) {
                            form.setValue("malePopulation", Math.max(0, total - val), { shouldValidate: true });
                          }
                        }}
            ...
            <div className="flex items-center space-x-2 bg-secondary/20 p-3 rounded-xl border border-border/40">
              <Switch
                id="auto-calculate-cohorts"
                checked={autoCalculate}
                onCheckedChange={handleAutoCalculateChange}
                data-testid="switch-auto-calculate"
              />
              <div className="space-y-0.5">
                <Label htmlFor="auto-calculate-cohorts" className="text-sm font-semibold cursor-pointer">
                  Auto-calculate target cohorts
                </Label>
                <p className="text-xs text-muted-foreground">
                  Estimate Under-1, Under-5, and Pregnant target groups using country demographic ratios
                </p>
              </div>
            </div>
